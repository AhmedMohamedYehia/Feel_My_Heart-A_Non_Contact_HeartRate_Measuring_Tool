    ...
    def select_best(self, classifiers, weights, training_data):
        ...
        for clf in classifiers:
            ...
            for i in range(len(weights)):
                correctness = abs(clf.classify(training_data[i][0]) - training_data[i][1])
                accuracy.append(correctness)
                error = error + weights[i] * correctness
...
class CascadeClassifier():
    ...
    def train(self, training):
        ...
        for i in range(len(training)): 
            if training[i][1] == 0:
                negative_examples.append(training[i])
            else:
                positive_examples.append(training[i])
        for classifers_size in self.layers:
            clf = ViolaAndJones(T=classifers_size)
            examples = positive_examples+negative_examples
            clf.pipeline(examples, len(positive_examples))
            self.clfs.append(clf)
            false_positives = []
            for i in range(len(negative_examples)):
                if self.classify(negative_examples[i][0]) == 1:
                    false_positives.append(negative_examples[i])
